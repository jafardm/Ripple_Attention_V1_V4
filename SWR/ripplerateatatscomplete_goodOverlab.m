function M1Blocks = ripplerateatatscomplete_goodOverlab(M1ripples,ripple_duration,idx)
...
M1Blocks = nan(size(M1ripples,2),8);
...
for ii = 1:size(M1ripples,2)
    ...
    for jj = idx
        if ~isempty(M1ripples{jj,ii})
            ...
            session(jj,1) = nblocks1/(SRFN*ripple_duration);
            session(jj,2) = nblocks2/(SRFW*ripple_duration);
            session(jj,3) = nblocks3/(LRFN*ripple_duration);
            session(jj,4) = nblocks4/(LRFW*ripple_duration);
            session(jj,5) = nblocks5/(SawN*ripple_duration);
            session(jj,6) = nblocks6/(SawW*ripple_duration);
            session(jj,7) = nblocks7/(LawN*ripple_duration);
            session(jj,8) = nblocks8/(LawW*ripple_duration);
        end
    end
